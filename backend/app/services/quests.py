...
# Притяжательное в роде биотопа: «Твой лес», «Твоё поле», «Твои пески». Склонять
# программно нечего — биотопов ровно десять, список закрытый.
_PERSONAL_BY_BIOTOPE = {
    "лес": "Твой лес", "луг": "Твой луг", "поле/сорное": "Твоё поле",
    "сады/парки": "Твой парк", "кустарники/заросли": "Твои заросли",
    "водное/прибрежное": "Твой берег", "болото/сырое": "Твоё болото",
    "каменистые/скалистые склоны": "Твои скалы", "пески/дюны/обнажения": "Твои пески",
    "горы/предгорья": "Твои горы",
}


def _personal_place_name(topo: dict | None, bios: set) -> str:
    """«Твой лес · Кролики» — притяжательное объясняет, откуда место взялось, биотоп
    отличает соседние места друг от друга, топоним привязывает к местности."""
    loc = (topo or {}).get("locality") or (topo or {}).get("green")
    owned = next((_PERSONAL_BY_BIOTOPE[b] for b in sorted(bios) if b in _PERSONAL_BY_BIOTOPE),
                 None) if bios else None
    if owned and loc:
        return f"{owned} · {loc}"
    if owned:
        return owned
    if loc:
        return f"Твоё место · {loc}"
    return "Твоё место"
