...
Все обращения к LLM идут через OpenRouter API. Модели переключаются в зависимости от задачи.

| Задача | Модель | Почему |
|--------|--------|--------|
| **Общая (по умолчанию)** | `qwen/qwen3-235b-a22b` | Основная модель для бота, агентов, нормализации текста |
| **OCR fallback (сложные сканы)** | `google/gemini-2.5-flash` | Быстрая, дешёвая, отличный multimodal — идеальна для "прочитай что на картинке" |
| **OCR сложные случаи** | `google/gemini-2.5-pro` | Для страниц с очень плохим качеством, нестандартными шрифтами, таблицами |
| **Структурное извлечение** | `qwen/qwen3-235b-a22b` | Парсинг рецептов в JSON, извлечение ингредиентов и мер |
| **Нормализация орфографии** | `qwen/qwen3-235b-a22b` | Перевод дореволюционного текста, конвертация мер |
| **Спеллчек / мелкие задачи** | `qwen/qwen3-32b` | Лёгкая модель для массовых операций (проверка слов, классификация чанков) |
| **Embeddings** | BGE-M3 (self-hosted) | Не через OpenRouter, свой сервис на :8100 |
...
# Модели по задачам (легко переключать)
LLM_MODELS = {
    "default": "qwen/qwen3-235b-a22b",
    "ocr_fallback": "google/gemini-2.5-flash",
    "ocr_hard": "google/gemini-2.5-pro",
    "extraction": "qwen/qwen3-235b-a22b",
    "normalization": "qwen/qwen3-235b-a22b",
    "lightweight": "qwen/qwen3-32b",
}

# BGE-M3 (self-hosted, отдельно)
BGE_M3_URL = env("BGE_M3_URL", "http://172.17.0.1")
BGE_M3_PORT = env("BGE_M3_PORT", 8100)
```

**Принцип выбора модели для OCR:**
- Tesseract — первый проход (бесплатно, быстро)
- Gemini 2.5 Flash — fallback для страниц с confidence < 60% (дёшево, multimodal)
- Gemini 2.5 Pro — для совсем сложных случаев (таблицы, нечитаемые шрифты, рукописный текст)
- Qwen 3 — не для OCR (нет сильного multimodal), но отлично для постобработки текста
...
│                                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐           │
│  │  BGE-M3       │  │  Nginx        │  │  OpenRouter   │          │
│  │  (Embeddings) │  │  (Proxy+SSL)  │  │  (LLM API)    │          │
│  │  :8100        │  │  :80/:443     │  │  external     │          │
│  └──────────────┘  └──────────────┘  └──────────────┘           │
└─────────────────────────────────────────────────────────────────┘
```
...
│   │   │   ├── llm.py           # OpenRouter клиент (выбор модели по задаче)
│   │   │   ├── embedder.py      # BGE-M3 клиент (self-hosted)
